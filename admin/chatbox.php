...
if (isset($_GET["chat"]) && !empty($_GET["chat"]) && $_GET["chat"] != "null") {
    $chat_id = $_GET["chat"];
    //verifier si le delegue existe
    $req = $db->prepare("SELECT * FROM delegue WHERE id_delegue = $chat_id");
    $req->execute();
    $delegue = $req->fetch(PDO::FETCH_ASSOC);
    if (empty($delegue)) {
        header("Location: index.php");
        exit;
    }
...
WHERE vu_admin = 0 AND vu_delegue = 1 AND message.id_delegue = $chat_id AND message.destinataire = 'admin'
ORDER BY id DESC
");
$req->execute();
...
if (isset($_POST["submit"])) {
    if (!empty($_POST["message"]) && !empty($_POST["message_id"])) {
        $req = $db->prepare("UPDATE message SET reponse = ?, h_reponse = current_timestamp(), vu_admin = 1 WHERE id = ?");

        if ($req->execute([$_POST["message"], $_POST["message_id"]])) {
            header("Location: chatbox.php?chat=$chat_id");
            exit;
        } else {
    ...
    if (!empty($_POST["message"])) {
        $req = $db->prepare("INSERT INTO message (id_delegue, id_admin, message, date, vu_admin, vu_delegue, destinataire, reponse, h_reponse) VALUES (?, ?, ?, current_timestamp(), '1', '0', 'delegue', NULL, NULL)");
        if ($req->execute([$chat_id, 1, $_POST["message"]])) {
            header("Location: chatbox.php?chat=$chat_id");
            exit;
        } else {
            echo "requette impossible";
...
<body>
    <div class="chat-container">
            ...
            if (!empty($value[$sender])) {
                if ($value[$date] < $value[$r_date]) {
                    ...
                    if (!empty($value[$recepteur])) {
                        ...
                        <p class="message-time ' . $message_class . '">' . $value[$r_date] . '</p>
                        </div>';
                    }
                }
            }
        ...
        if (count($messages) <= 0) { ?>
            <form action="chatbox.php?chat=<?php echo $chat_id ?>" method="post">
    ...
    <div>
        <a href="index.php" class="quitter">Retour à l'accueil</a>
    </div>
    <script>
